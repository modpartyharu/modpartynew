...
class SyncService(
    ...
    // =================================================================
    // 기능5: DB 우선(regionName) + 동기화 대상 확장(siteCode, prodNo)
    // - regionName = DB 매핑 우선, 없으면 RegionConfig fallback
    // - 동기화 대상 = RegionConfig에 있거나, DB 매핑이 활성화되어 있으면 포함
    // =================================================================
    ...
    private fun getRegionNameDbFirst(siteCode: String, prodNo: Int?): String? {
        return getDbRegionName(siteCode, prodNo) ?: regionConfig.getRegionName(prodNo)
    }

    private fun isSyncTarget(siteCode: String, prodNo: Int?): Boolean {
        ...
        try {
            ...
            while (hasMore) {
                ...
                for (order in orderList) {
                    ...
                    // 기능5: 동기화 대상 확장 (RegionConfig OR DB 매핑)
                    if (!isSyncTarget(siteCode, firstProdNo)) {
                        logger.debug("Skipping order {} - prodNo {} is not in sync target (RegionConfig/DB)", order.orderNo, firstProdNo)
                        continue
                    }
    ...
    private suspend fun processOrder(
        ...
        return SyncOrder(
            ...
            // 기능5: regionName DB 우선(매핑) + fallback(RegionConfig)
            regionName = getRegionNameDbFirst(siteCode, prodNo),
